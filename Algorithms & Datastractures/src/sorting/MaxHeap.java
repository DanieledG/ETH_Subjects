...
public class MaxHeap extends Heap{
	...
	public void restoreHeapCondition(IntNode n) {
			IntNode current = n;
 			if (current == root) {
 				while ((current.left != null) && (current.right != null) && ((current.key < current.right.key) || (current.key < current.left.key))) {
 			...
 			if (!checkHeapCondition(root)) {
 				System.out.println("ERROR MAXHEAP");
 			}
 		}
